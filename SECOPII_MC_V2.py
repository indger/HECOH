...
fi = (datetime.now() + timedelta(days=-0)).strftime('%d/%m/%Y')
ff = (datetime.now() + timedelta(days=1)).strftime('%d/%m/%Y')
...
# , 'Selección Abreviada de Menor Cuantia sin Manifestacion de Interés'
# Lista con los tipos de procesos a busar
minima = ['Mínima cuantía', 'Selección abreviada menor cuantía']
pre_table = []
...
resultados_total_SII_MC = pd.read_excel(PATH_SII + 'resultados_total_SII_MC.xls', header = 0)

#Cruzar base de resultados total, con la bse de resultados del dia
tabla_ini = pd.merge(tabla_ini,resultados_total_SII_MC, 
                              how = 'left', 
                              on = ['Link','Entidad'])
...
resultados_MC_hoy_SII = tabla_com[(tabla_com.Estado == "Published") 
                          & (tabla_com.Cuantia > 6000000)]
    
# Incluir los resultados nuevos del día, en el historico de resultados total
resultados_total_SII_MC = resultados_total_SII_MC.append(tabla_com)

# Exportar en excel las tablas de resultados total y resultados del dia
resultados_total_SII_MC.to_excel(PATH_SII + 'resultados_total_SII_MC.xls', index = False)
resultados_MC_hoy_SII.to_excel(PATH_SII + 'resultados_MC_hoy_SII' + ar +'.xls', index = False)

#Enviar email con los resultados del día actualizados

asunto = 'Resultados SECOP II M.C.' + ar
COMMASPACE = ', '
...
def main():
    ...
    body = 'Se encontraron los siguientes resultados para procesos de Minima cuantia en el SECOP II'
    ...
    # List of attachments
    attachments = [PATH_SII + 'resultados_MC_hoy_SII' + ar + '.xls']
